...
cp "$BINARY" "$DATA_DIR/usr/lib/hoki-audiobook"
patchelf --set-interpreter /lib/ld-linux-armhf.so.3 --set-rpath /usr/lib:/lib "$DATA_DIR/usr/lib/hoki-audiobook"
cp "$SRCDIR/deploy/hoki-audiobook" "$DATA_DIR/usr/bin/hoki-audiobook"
chmod 755 "$DATA_DIR/usr/bin/hoki-audiobook"
cp "$SRCDIR/deploy/hoki-audiobook.desktop" "$DATA_DIR/usr/share/applications/"
...
Package: ${PKG}
Version: ${VERSION}-r0
Description: Audiobook player for AsteroidOS
Section: multimedia
Priority: optional
Maintainer: 0x53a
License: MIT
Architecture: ${ARCH}
Depends: libc6, gstreamer1.0
...
#!/bin/sh
set -e
killall hoki-audiobook 2>/dev/null || true
EOF
